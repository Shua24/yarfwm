...
bool Config::write_default_if_missing() const {
    ...
    // Dynamic memory: the default configuration text is a static string that
    // lives in the executable image; it is streamed straight to the new file.
...
bool Config::load(const std::string &path) {
    ...
    // No usable configuration file exists yet: write the built-in default to
    // $HOME/.config/yarfwm/config.json once, then load it.
