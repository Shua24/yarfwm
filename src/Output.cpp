...
Output::Output()
    : river_output(nullptr), x(0), y(0), width(0), height(0),
      has_dimensions(false), has_non_exclusive_area(false),
      non_exclusive_area_x(0), non_exclusive_area_y(0),
      non_exclusive_area_width(0), non_exclusive_area_height(0),
      layer_shell_output(nullptr), view(nullptr)
{
}

Output::~Output() { terminate(); }

bool Output::initialize(LayerShell &layer_shell, struct river_output_v1 *output,
			View *owner)
{
	river_output = output;
	view = owner;
	...
	static const struct river_output_v1_listener output_listener = {
	    river_output_removed,	   river_output_wl_output,
	    river_output_position,	   river_output_dimensions,
	...
	if (layer_shell.available()) {
		...
		if (layer_shell_output) {
			...
			if (river_layer_shell_output_v1_add_listener(
				layer_shell_output,
				&layer_shell_output_listener, this) != 0) {
				std::fprintf(stderr,
...
void Output::terminate()
{
	...
	// river_output_v1.removed event and is destroyed before the output
	// itself.
	if (layer_shell_output) {
...
void Output::river_output_wl_output(void *data, struct river_output_v1 *output,
				    uint32_t name)
{
	(void)data;
	(void)output;
...
void Output::river_output_capture_sessions(void *data,
