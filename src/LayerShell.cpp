...
void LayerShell::bind_registry_global(struct wl_registry *registry, uint32_t name, uint32_t version) {
    if (layer_shell) {
        return;
    }
    ...
    if (!layer_shell) {
        std::fprintf(stderr, "Yarfwm: failed to bind river_layer_shell_v1\n");
    }
}

bool LayerShell::available() const {
    return layer_shell != nullptr;
}

struct river_layer_shell_output_v1 *LayerShell::create_output_state(struct river_output_v1 *output) {
    if (!layer_shell) {
        return nullptr;
    }
...
struct river_layer_shell_seat_v1 *LayerShell::create_seat_state(struct river_seat_v1 *seat) {
    if (!layer_shell) {
        return nullptr;
    }
