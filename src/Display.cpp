...
static void display_handle_destroy(struct wl_listener *listener, void *data) {
    ...
    // terminate() from inside a dispatch callback. Shutdown now travels through
    // View::shutdown_requested and the main loop, so this stays unused.
}
...
static void registry_handle_global(void *data, struct wl_registry *registry,
    ...
    if (std::strcmp(interface, "river_window_manager_v1") == 0) {
        display->window_manager = static_cast<struct river_window_manager_v1 *>(
            wl_registry_bind(registry, name, &river_window_manager_v1_interface,
        ...
        display->layer_shell_state.bind_registry_global(registry, name, version);
    }
}
...
bool Display::roundtrip(Server *server) {
    ...
    // C6: one bounded roundtrip so the registry globals arrive before we decide
    // what to bind. The old unbounded dispatch loop never returned.
    return wl_display_roundtrip(server->display) >= 0;
}

void Display::terminate() {
    // Children first: the View and the Seat destroy their layer shell output
    // and seat state objects in their own terminate() before this runs (see
    // Yarfwm::terminate()), so the global is the last layer shell object left.
